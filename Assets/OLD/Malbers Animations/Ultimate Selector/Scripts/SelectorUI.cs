...
namespace MalbersAnimations.Selector
{
    public class SelectorUI : MonoBehaviour
    {
        ...
        public List<GameObject> PurchaseItem = new List<GameObject>();
        int PriseAmount;
        int purch = 0;
        int p;
        [Space]
        ...
        private void Start()
        {
            ...
            for(int i = 0; i < p; i++)
            {
                PurchaseItem[i].transform.GetChild(7).GetChild(1).gameObject.SetActive(false);
                PurchaseItem[i].GetComponent<MItem>().Locked = false;
                if(PurchaseItem[i].GetComponent<MItem>().Locked == false)
                {
        ...
        public virtual void Purchase()
        {
          ...
          //   CoinSystem.inst.Purchasing(PriseValue);

            Debug.Log("PriseValue" + PriseAmount);
          ...
          //  {
                if (CoinSystem.inst.Coin >= PriseAmount)         //If you have more coins than the Item value (then you can make a purchase)
                {
      
                CoinSystem.inst.Purchasing(PriseAmount);
                
        
                Manager.Purchase(ItemSelected);
             ;//Call the Main Purchase Method from the Selector Manager
              //UpdateSelectedItemUI(ItemSelected.gameObject);
        
                ItemSelected.RemoveLockMaterial();
       
                ItemSelected.Locked = false;
               
           
                ItemSelected.transform.GetChild(7).GetChild(1).gameObject.SetActive(false);

                purch = PlayerPrefs.GetInt("items");
             
                purch++;
                PlayerPrefs.SetInt("items",purch);
        ...
        public virtual void SelectItem()
        {
            ...
            else                                                                            //Else show on the Data is locked
            {
                ...
                TextItemData.text = SelectLocked;
            }
        }
